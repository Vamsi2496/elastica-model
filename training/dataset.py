...
def _compute_d_weights(d_phys: np.ndarray, n_bins: int) -> np.ndarray:
    """Return per-sample importance weights so every d-bin trains equally.

    Weight = total_samples / (n_bins * count_in_bin).  Samples in sparse
    high-d bins (near the snapping boundary) get proportionally higher weight
    so the model receives equal gradient signal across the full d range.
    """
    ...
    weights = 1.0 / counts[bin_idx]          # inverse-frequency weight
...
class ElasticaDataset:
    def __init__(self, path: str, compute_stats: bool = True):
        ...
        # store raw physical d for weight computation in get_loaders
...
def get_loaders(path: str, compute_stats: bool = True):
    ...
    train_ds = TensorDataset(dataset.x[train_idx], dataset.y[train_idx],
                             dataset.arc[train_idx], dataset.theta[train_idx])
    val_ds   = TensorDataset(dataset.x[val_idx],   dataset.y[val_idx],
                             dataset.arc[val_idx],   dataset.theta[val_idx])
    test_ds  = TensorDataset(dataset.x[test_idx],  dataset.y[test_idx],
                             dataset.arc[test_idx],  dataset.theta[test_idx])
