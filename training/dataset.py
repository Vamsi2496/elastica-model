...
class ElasticaDataset:
    def __init__(self, path: str, compute_stats: bool = True):
        ...
        with h5py.File(path, "r") as f:
            ...
            print(
                f"  Dataset on GPU: "
                f"{(self.x.numel() + self.y.numel() + self.arc.numel() + self.theta.numel()) * 4 / 1e9:.2f} GB"
            )
...
def get_loaders(path: str, compute_stats: bool = True):
    ...
    train_ds = TensorDataset(
        dataset.x[train_idx], dataset.y[train_idx], dataset.arc[train_idx], dataset.theta[train_idx]
    )
    val_ds = TensorDataset(
        dataset.x[val_idx], dataset.y[val_idx], dataset.arc[val_idx], dataset.theta[val_idx]
    )
    test_ds = TensorDataset(
        dataset.x[test_idx], dataset.y[test_idx], dataset.arc[test_idx], dataset.theta[test_idx]
    )
