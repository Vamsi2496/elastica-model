...
class ElasticaDataset:
    def __init__(self, path: str, compute_stats: bool = True):
        ...
        with h5py.File(path, "r") as f:
            ...
            phi1 = f[Config.KEY_PHI1][:].astype(np.float64)
            phi2 = f[Config.KEY_PHI2][:].astype(np.float64)
            d = f[Config.KEY_D][:].astype(np.float64)
            arc = np.vstack(f[Config.KEY_ARC][:]).astype(np.float64)
            theta = np.vstack(f[Config.KEY_THETA][:]).astype(np.float64)
            params = f[Config.KEY_PARAMS][:].astype(np.float64)
        ...
        if compute_stats:
            ...
            self.x_mean = X.mean(0).astype(np.float64)
            self.x_std = X.std(0).astype(np.float64) + 1e-8
            self.y_mean = Y.mean(0).astype(np.float64)
            self.y_std = Y.std(0).astype(np.float64) + 1e-8
...
def get_loaders(path: str, compute_stats: bool = True):
    ...
    def make_loader(ds, shuffle):
        return DataLoader(ds, batch_size=Config.BATCH_SIZE, shuffle=shuffle, num_workers=Config.NUM_WORKERS, pin_memory=Config.PIN_MEMORY)
    return make_loader(train_ds, True), make_loader(val_ds, False), make_loader(test_ds, False), dataset
