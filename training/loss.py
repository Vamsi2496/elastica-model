...
class ElasticaLoss:
    ...
    def _deriv2(f, arc):
        ...
        h1 = h[:, :-1]; h2 = h[:, 1:]
        df_ds[:, 1:-1] = (h1**2 * f[:, 2:] - h2**2 * f[:, :-2]
                          - (h1**2 - h2**2) * f[:, 1:-1]) / (h1 * h2 * (h1 + h2)).clamp(min=1e-8)
        h0 = h[:, 0]; h1b = h[:, 1]
        df_ds[:, 0] = (-(2*h0+h1b)/(h0*(h0+h1b)).clamp(min=1e-8)*f[:, 0]
                       +(h0+h1b)/(h0*h1b).clamp(min=1e-8)*f[:, 1]
                       -h0/(h1b*(h0+h1b)).clamp(min=1e-8)*f[:, 2])
        hN2 = h[:, -2]; hN1 = h[:, -1]
        df_ds[:, -1] = (hN1/(hN2*(hN1+hN2)).clamp(min=1e-8)*f[:, -3]
    ...
    def __call__(self, model, x, y, arc_norm, theta_true, need_stiffness=False):
        ...
        energy_true, fx_true, fy_true, m1_true, m2_true = (
            y[:, 0], y[:, 1], y[:, 2], y[:, 3], y[:, 4])
        ...
        loss_scalar = (Config.FX_WEIGHT    * mse_fx
                       + Config.FY_WEIGHT  * mse_fy
                       + Config.M_WEIGHT   * mse_m1
                       + Config.M_WEIGHT   * mse_m2
